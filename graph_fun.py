...
#graph building stuff that's not used
    ...
    for i in range(len(adj)):
        for j in range(len(adj)):
            if distance == 'Euclidean':
                dis = np.exp(distance_est.euclidean_distance(features[i], features[j])*-1)
                if dis >= thre:
                    adj[i, j] = dis
            elif distance == 'cos':
                dis = distance_est.cos_distance(features[i], features[j])
                if dis >= thre:
                    adj[i, j] = dis
            elif distance == 'pearson_correlation':
                dis = distance_est.pearson_correlation(features[i], features[j])
                if dis >= thre:
                    adj[i, j] = dis
    return adj
...
#i don't think all these things are used
    ...
    for index1, feature1 in enumerate(features):
        for index2, feature2 in enumerate(features):
            if labels[index1] == labels[index2]:
                distance_list[labels[index1]] += distance_est.euclidean_distance(features[index1], features[index2])
...
def radius_graph(features, dis=None, labels=None, class_number=None, include_self=True, limit_radius=False):
    ...
    for index1, feature1 in enumerate(features):
        for index2, feature2 in enumerate(features):
            if dis >= distance_est.euclidean_distance(feature1, feature2) and index1 != index2:
                adjacency_matrix[index1, index2] = 1
            elif dis >= distance_est.euclidean_distance(feature1, feature2) and index1 == index2 and include_self:
                adjacency_matrix[index1, index2] = 1
